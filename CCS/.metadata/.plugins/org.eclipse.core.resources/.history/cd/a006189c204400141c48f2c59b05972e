...
void SetTimerA(int freq, int duty)
{
	TACCR0 = freq - 1; //основной ограничитель
	TACCR1 = duty; //для того что бы рабочий чикл был равен 50%
}

void SetTimers(int sym)
{
	TACTL = MC_0; //стоп таймера
	...
	TACTL = TASSEL_1 + MC_1; //запуск таймера с параметрами вверх и ACLK
